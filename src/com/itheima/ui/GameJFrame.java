...
import com.itheima.domain.gameif;
...
public class GameJFrame extends JFrame implements KeyListener, ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (obj == replayItem) {
            ...
            //写入文件
            try {
                ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(new File("src\\"+index+".data")));
                gameif g = new gameif(data,x,y,path,step);
                IoUtil.writeObj(oos,true,g);
            } catch (IOException ex) {
                throw new RuntimeException(ex);
            }
            //修改显示信息
            item.setText("存档"+index+"("+step+")");
            ...
            try {
                ObjectInputStream ois = new ObjectInputStream(new FileInputStream(new File("src\\"+index+".data")));
                g = (gameif) ois.readObject();
                ois.close();
            } catch (IOException ex) {
                throw new RuntimeException(ex);
